  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <Title level={4} style={{ margin: 0 }}>{isEdit ? '编辑规则' : '新建规则'}</Title>
        <Button
      ...
      <Card style={{ maxWidth: 800 }}>
        ...
        >
          ...
          <Form.Item
            name="lark_config_id"
            label="Lark 告警配置"
            extra="选择已配置的 Lark 配置，或选择空直接输入 URL"
          >
            <Select
              allowClear
          ...
          >
            {({ getFieldValue }) => {
              ...
              return (
                <Form.Item
                  name="lark_webhook"
                  label={`Lark Webhook URL ${hasConfig ? '(可选)' : ''}`}
                  rules={[{
                    required: !hasConfig,
                    message: '请选择 Lark 配置或输入 Webhook URL',
                  }]}
                >
                  <Input
