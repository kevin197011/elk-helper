...
func (e *Executor) sendAlertAsync(ruleModel *models.Rule, logs []map[string]interface{}, fromTime, toTime time.Time, timeRange string) {
	...
	go func() {
		err := notifier.SendAlert(ruleModel.Name, ruleModel.IndexPattern, logs, fromTime, toTime, e.retryTimes)
		alertErrChan <- err
	}()
	...
	case <-time.After(30 * time.Second):
		...
		slog.Error("Alert send timeout", "rule_id", ruleModel.ID, "rule_name", ruleModel.Name)
	}
